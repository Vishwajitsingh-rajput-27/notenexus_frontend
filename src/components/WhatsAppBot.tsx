...
function CountdownTimer({ seconds, onExpire }: { seconds: number; onExpire: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
      ...
      <span style={{ fontFamily: mono, fontSize: 11, color, minWidth: 36 }}>
        {mm}:{ss}
      </span>
    </div>
  )
...
export default function WhatsAppBot() {
  ...
  const fetchLinkStatus = useCallback(() => {
    if (!token) return
    fetch(`${API}/api/whatsapp/link-status`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
      .then(setLinkStatus)
      .catch((err) => console.error('[fetchLinkStatus]', err))
  }, [token])
  ...
  async function generateCode() {
    ...
    if (!token) {
      ...
      console.error('[generateCode] No token found')
      return
    }
    ...
    try {
      console.log('[generateCode] Sending request...')

      const response = await fetch(`${API}/api/whatsapp/generate-link-code`, {
        method: 'POST',
        headers: {
      ...
      console.log('[generateCode] Response:', response.status, data)

      if (!response.ok) {
        setError(data.message || 'Failed to generate code')
        console.error('[generateCode] Server error:', data)
        return
      }

      if (data.code) {
        setLinkCode(data.code)
        console.log('[generateCode] ✅ Code received:', data.code)
      } else {
        setError('No code received from server')
        console.error('[generateCode] No code in response:', data)
      }
    } catch (err: any) {
    ...
    <div style={{ maxWidth: 700 }}>
      ...
      >
        ...
        <span
          style={{
            ...
            fontWeight: 700,
            letterSpacing: '0.08em',
            color: status?.configured ? '#4ADE80' : '#FBFF48',
          }}
        >
          {status?.configured ? 'BOT_STATUS: LIVE' : 'BOT_STATUS: SETUP_REQUIRED'}
        </span>
        {status?.configured && (
          <span style={{ fontFamily: mono, fontSize: 10, color: t.fgDim, marginLeft: 'auto' }}>
            +1 415 523 8886
          </span>
        )}
      </div>
        ...
        <div style={{ ...card, marginBottom: 24 }}>
          ...
          {STEPS.map(({ n, title, detail }) => (
            <div key={n} style={{ display: 'flex', gap: 16, padding: '12px 18px', borderBottom: `1px solid ${t.border}` }}>
              <div style={{ fontFamily: mono, fontSize: 10, color: t.accent, flexShrink: 0, width: 24, paddingTop: 2 }}>
                {n}
              </div>
              <div>
                <div style={{ fontFamily: mono, fontSize: 11, color: t.fg, fontWeight: 700, marginBottom: 3 }}>
                  {title}
                </div>
          ...
          {linkStatus?.linked ? (
            ...
            <div>
              ...
              <div style={{ fontFamily: ibm, fontSize: 12, color: t.fgDim, lineHeight: 2, marginBottom: 18 }}>
                1. Click <em>Generate Link Code</em> below
                <br />
                2. Open WhatsApp → message <strong>+1 415 523 8886</strong>
                <br />
                3. Send:{' '}
                <code style={{ fontFamily: mono, color: '#4ADE80', background: 'rgba(74,222,128,0.08)', padding: '1px 6px' }}>
                  link {'<'}CODE{'>'}
                </code>
              </div>
              ...
              {linkCode ? (
                <div>
                  ...
                  >
                    <div style={{ fontFamily: mono, fontSize: 10, color: t.fgDim, letterSpacing: '0.12em', marginBottom: 8 }}>
                      YOUR_LINK_CODE
                    </div>
                    <div style={{ fontFamily: mono, fontSize: 28, fontWeight: 700, color: '#4ADE80', letterSpacing: '0.25em' }}>
                      {linkCode}
                    </div>
                ...
                <button
                  style={{ ...btn, background: '#4ADE80', color: '#0a0a0a', opacity: codeLoading ? 0.7 : 1 }}
                  onClick={generateCode}
                  disabled={codeLoading}
                >
                ...
                {section.commands.map((c, i) => (
                  <div
                    ...
                    style={{
                      display: 'flex',
                      gap: 0,
                      borderBottom: i < section.commands.length - 1 ? `1px solid ${t.border}` : 'none',
                    }}
                  >
                    <code
                      style={{
      ...
      {status?.webhookUrl && (
        <div style={{ ...card, padding: '12px 18px', marginTop: 12 }}>
          <div style={{ fontFamily: mono, fontSize: 9, color: t.fgDim, letterSpacing: '0.12em', marginBottom: 6 }}>
            // WEBHOOK_URL
          </div>
