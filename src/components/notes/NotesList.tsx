          ...
          {['', ...subjects].map(s => (
            <button key={s || 'all'} onClick={() => setFilter(s)}
              style={{
                ...
                border: `1px solid ${filter === s ? '#FBFF48' : t.border}`,
          ...
          <AnimatePresence>
            {notes.map((note, i) => (
              <motion.div key={note._id}
                ...
                style={{ background: t.bg2, borderLeft: expanded === note._id ? '2px solid #FBFF48' : '2px solid transparent', transition: 'border-color 0.2s' }}>
